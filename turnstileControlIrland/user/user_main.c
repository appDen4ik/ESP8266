...
wdt_timeout( void ) {
	...
	os_timer_setfn( &wdt_timer, (os_timer_func_t *)wdt_timeout, (void *)0 );
	os_timer_arm( &wdt_timer, 500, 0 );
}
...
init_done( void ) {
	...
	softapConf->ssid_len = os_sprintf( softapConf->ssid, "%s", &flashTmp[SSID_AP_OFSET] );
	os_printf( " softapConf->ssid  %s, softapConf->ssid_len  %d\r\n", softapConf->ssid, softapConf->ssid_len );

	os_sprintf( softapConf->password, "%s", &flashTmp[PWD_AP_OFSET] );
	...
	gpio_pin_intr_state_set( GPIO_ID_PIN( 12 ), 4 ); // Interrupt on lowlevel GPIO edge
...
//		flashTmp[ 1 + NUMBER_OF_TURNSTILES_OFSET ] = '\n';
		...
		memcpy( &flashTmp[PWD_AP_OFSET], DEF_PWD_AP, sizeof(DEF_PWD_AP) );
		flashTmp[ sizeof(DEF_PWD_AP) + PWD_AP_OFSET ] = '\n';

		memcpy( &flashTmp[HEADER_AP_OFSET], HEADER_AP, sizeof(HEADER_AP) );
		flashTmp[ sizeof(HEADER_AP) + HEADER_AP_OFSET ] = '\n';
...
//=================================================================================================
	...
	struct softap_config myStruct;
	wifi_softap_get_config(&myStruct);
